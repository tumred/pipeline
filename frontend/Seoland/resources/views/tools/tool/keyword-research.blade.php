    ...
    <div class="card-body">
        ...
        <form action="{{ route('tools.keyword_research')  }}" method="post" enctype="multipart/form-data" id="keywords-research-form">
            ...
            <div class="form-group">
                ...
                <textarea name="keywords" id="i-keywords" class="form-control{{ $errors->has('keywords') ? ' is-invalid' : '' }}" rows="3" placeholder="{{ __('One per line') }}">{{ $keywords ?? (old('keywords') ?? '') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select name="country" id="i-country" class="custom-select{{ $errors->has('country') ? ' is-invalid' : '' }}">
                    @foreach(['' => __('All'), 'au' => 'Australia', 'ca' => 'Canada', 'in' => 'India', 'nz' => 'New Zealand', 'za' => 'South Africa', 'uk' => 'United Kingdom', 'us' => 'United States'] as $key => $value)
                        <option value="{{ $key }}" @if ((isset($country) && $key == $country) || (old('country') !== null && $key == old('country')) || (empty($key) && old('country'))) selected @endif>{{ $value }}</option>
                    @endforeach
                </select>
                    ...
                    @foreach($results['data'] as $keyword)
                        <div class="list-group-item px-0">
                            <div class="row align-items-center">
                                ...
                                <div class="col-auto">
                                    <div class="btn btn-sm btn-outline-primary keyword-copy" data-enable="tooltip-keyword" data-value="{{ $keyword['keyword'] }}" title="{{ __('Copy') }}" data-copy="{{ __('Copy') }}" data-copied="{{ __('Copied') }}" data-clipboard-target="#i-keyword-{{ Str::slug($keyword['keyword']) }}">{{ __('Copy') }}</div>
                                </div>
                            </div>
                        </div>
                    @endforeach
